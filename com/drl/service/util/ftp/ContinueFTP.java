package com.drl.service.util.ftp;
import java.io.File;
...
public class ContinueFTP{
    private static ContinueFTP myFtp = new ContinueFTP();
    
    public static ContinueFTP getInstance() {
    	try
        {
            myFtp.connect("172.16.4.122",21, "ftpUser1", "123");
        }
        catch (Exception e)
        {
            e.printStackTrace();
        ...
        throws Exception
    {
        ...
        {
            throw new Exception("登陆异常，请查看主机端口");
        }
        ...
        if (FTPReply.isPositiveCompletion(ftpClient.getReplyCode()))
        {
            if (ftpClient.login(username, password))
            {
                ...
                throw new Exception("登陆异常，请查看密码账号");
        }
        else
     ...
     * @throws IOException
     */
    public List getFileList(String filedir)
        throws IOException
    {
        ...
        if (files != null)
        {
            ...
            for (int i = 0; i < files.length; i++)
            {
            	...
            	}else if(files[i].getType() == FTPFile.DIRECTORY_TYPE) {
     ...
     * @throws IOException
     */
    public void downloadAll(String remote, String local) throws IOException {
		ContinueFTP myFtp =ContinueFTP.getInstance();
		local=local+"\\"+remote.substring(remote.lastIndexOf("/")+1)+"\\\\";
		List fileList = myFtp.getFileList(remote);
		if(fileList==null) {
			System.out.println("kong");
			myFtp.download("", local);
			return;
		}
		List<String> sfiles=(List<String>) fileList.get(0);
		if(sfiles.size()!=0) {
			for (String sfile : sfiles) {
				System.out.println("下载："+remote+"/"+sfile+"到："+local);
				myFtp.download(remote+"/"+sfile, local+sfile);
			}
		}
		List<String> dirs=(List<String>) fileList.get(1);
		for (String dir : dirs) {
			System.out.println("进入目录："+remote+"/"+dir);
			myFtp.downloadAll(remote+"/"+dir,local);
		}
	}
     ...
     * 上传文件到服务器,新上传和断点续传
     * 
     * @param remoteFile 远程文件名，在上传之前已经将服务器工作目录做了改�?
     * @param localFile 本地文件File句柄，绝对路�?
     * @param processStep �?要显示的处理进度步进�?
